...
describe('Team e2e test', () => {
    ...
    it('should load Teams', () => {
        entityMenu.click();
        element.all(by.css('[routerLink="team"]')).first().click().then(() => {
            const expectVal = /Teams/;
    ...
    it('should load create Team dialog', function () {
        element(by.css('button.create-team')).click().then(() => {
            const expectVal = /Create or edit a Team/;
